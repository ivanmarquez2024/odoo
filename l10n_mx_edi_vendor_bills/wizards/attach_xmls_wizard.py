...
class AttachXmlsWizard(models.TransientModel):
        ...
        for index, rec in enumerate(xml.Conceptos.Concepto):
            ...
            for tax in taxes:
                ...
                if not tax_group_id or not tax_get:
                    ...
                    continue
                tax_line = (tax_get.invoice_repartition_line_ids).filtered(lambda x: x.repartition_type == 'tax')
                if not tax_line.account_id.id:
                    taxes_list['withno_account'].append(
                        name if name else tax['tax'])
                else:
                    tax['id'] = tax_get.id
                    tax['account'] = tax_line.account_id.id
    ...
    def create_invoice(
        ...
        invoice_id.l10n_mx_edi_cfdi = xml_str
