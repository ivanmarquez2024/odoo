...
class MrpAbstractWorkorder(models.AbstractModel):
    ...
    def _update_finished_move(self):
        ...
        for move_production in production_move:
            if move_production and move_production.product_id.tracking != 'none':
                if not self.finished_lot_id:
                    raise UserError(_('You need to provide a lot for the finished product.'))
                move_line = move_production.move_line_ids.filtered(
                ...
                if move_line:
                    ...
                    location_dest_id = move_production.location_dest_id._get_putaway_strategy(self.product_id).id or move_production.location_dest_id.id
                    move_line.create({
                        'move_id': move_production.id,
                        'product_id': move_production.product_id.id,
                        'lot_id': self.finished_lot_id.id,
                        'product_uom_qty': self.qty_producing,
                        'product_uom_id': self.product_uom_id.id,
                        'qty_done': self.qty_producing,
                        'location_id': move_production.location_id.id,
                        'location_dest_id': location_dest_id,
                    })
            else:
                rounding = move_production.product_uom.rounding
                move_production._set_quantity_done(
