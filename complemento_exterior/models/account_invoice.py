...
class AccountMove(models.Model):
    ...
    cce_mercancias = fields.One2many('account.invoice.mercancias', 'order_id', 'Mercancias Complemento Exterior',
        copy=True, readonly=False,
        states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    cce_tipocambio = fields.Char(string=_('TipoCambio DOF'))

    @api.onchange('cce_habilitar_cee', 'cce_habilitar_exterior')
    def _get_exchange_rate(self):
        for inv in self:
           if inv.cce_habilitar_cee or inv.cce_habilitar_exterior:
              usd = inv.env["res.currency"].search([('name', '=', 'USD')], limit=1)
              inv.cce_tipocambio = inv.set_decimals(1 / usd.with_context(date=inv.invoice_date).rate, 4)
