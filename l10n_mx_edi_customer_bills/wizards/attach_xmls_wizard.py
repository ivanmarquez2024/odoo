...
class AttachXmlsWizard(models.TransientModel):
    ...
    def get_impuestos(self, xml):
        ...
        for index, rec in enumerate(xml.Conceptos.Concepto):
            ...
            for tax in taxes:
                ...
                tax_get = False
                for tax1 in taxes_get:
                    tax_line2 = (tax1.invoice_repartition_line_ids).filtered(lambda x: x.repartition_type == 'tax')
                    if not set(tax_line2.tag_ids.ids).isdisjoint(tax_tag_id.ids):
                        tax_get = tax1
                        break
                ...
                if not tax_tag_id or not tax_get:
                    ...
                    if not tax_line2.account_id.id:
                        taxes_list['withno_account'].append(
                            name if name else tax['tax'])
                    else:
                        tax['id'] = tax_get.id
                        tax['account'] = tax_line2.account_id.id
    ...
    def validate_documents(self, key, xml, account_id):
        ...
        inv = inv_obj
        inv_id = False
        xml_str = etree.tostring(xml, encoding='UTF-8')
        ...
        domain.extend(['|', ('supplier_rank', '>=', 1), ('customer_rank', '>=', 1)])
    ...
    def create_invoice(self, xml, supplier, currency_id, taxes, account_id=False):
        ...
        if xml.get('version') == '3.2':
            ...
            for global_tax in xml.xpath(tax_path, namespaces=xml.nsmap):
                ...
                tax_domain = [
                    ...
                    ('amount_type', '=', 'percent'),
                    ('amount', '=', tax_percent),
                    ('invoice_repartition_line_ids.tag_ids.name', '=', tax_name),
                ]
